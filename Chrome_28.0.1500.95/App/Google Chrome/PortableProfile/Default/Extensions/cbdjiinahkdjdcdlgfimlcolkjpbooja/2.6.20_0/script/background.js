...
chrome.extension.onRequest.addListener( 
  function request(request, sender, sendResponse) { 
    if (request.method == "completerequest") { sendResponse({nstyle: localStorage['nstyle'], cstyle: localStorage['cstyle'], uhd: localStorage['4k'], huhd: localStorage['2k'], fhd: localStorage['1080p'], hd: localStorage['720p'], hq: localStorage['480p'], mq: localStorage['360p'], lq: localStorage['240p'], ytmp3: localStorage['ytmp3'], v2mp3: localStorage['v2mp3'], uduhd: localStorage['ud4k'], udhuhd: localStorage['ud2k'], udfhd: localStorage['ud1080p'], udhd: localStorage['ud720p'], udhq: localStorage['ud480p'], udmq: localStorage['ud360p'], udlq: localStorage['ud240p'], version: localStorage['version'], username: localStorage['username'], quality: localStorage['quality'],/* webm: localStorage['webm'],*/ audio: localStorage['ytaudio']}); 
    } 
    else { 
      sendResponse({}); 
    } 
